...
# ---------------------------------------------------------------------------
# Extended threat status helpers (owner, due_date, list all)
# ---------------------------------------------------------------------------
def upsert_threat_status(threat_model_id: int, threat_id: str, status: str,
                          notes: str | None = None, updated_by: int = None,
                          owner: str | None = None, due_date: str | None = None) -> dict:
    """upsert_threat_status with optional owner + due_date columns.
    Falls back to calling the existing upsert_threat_status if owner/due_date not
    supported by the schema (safe degradation).
    """
    # Try to add columns if they don't exist yet (idempotent)
    with db_conn(write=True) as c:
        try:
            c.execute("ALTER TABLE threat_status ADD COLUMN owner TEXT")
        except Exception:
            pass
        try:
            c.execute("ALTER TABLE threat_status ADD COLUMN due_date TEXT")
        except Exception:
            pass

    result = set_threat_status(threat_model_id, threat_id, status, notes or "", updated_by or 0)

    # Update owner / due_date separately
    if owner is not None or due_date is not None:
        ...
        with db_conn(write=True) as c:
            c.execute(
                f"UPDATE threat_status SET {', '.join(updates)} WHERE threat_model_id=? AND threat_id=?",
                vals
            )
...
def list_threat_statuses(threat_model_id: int) -> list[dict]:
    """List all per-threat statuses for a threat model, including owner and due_date."""
    with db_conn() as c:
        rows = c.execute(
            "SELECT * FROM threat_status WHERE threat_model_id=? ORDER BY updated_at DESC",
            (threat_model_id,)
        ).fetchall()
    return [dict(r) for r in rows]
    ...
    with db_conn(write=True) as c:
        c.execute("""
            INSERT INTO custom_threat_rules
              (user_id, name, description, category, title, severity, applies_to, mitigations, tags)
            VALUES (?,?,?,?,?,?,?,?,?)
        """, (
            user_id,
            ...
            _json.dumps(data.get("applies_to", [])),
            _json.dumps(data.get("mitigations", [])),
...
def get_custom_rule(rule_id: int) -> dict | None:
    ...
    return _parse_custom_rule(dict(row))

def list_custom_rules(user_id: int, enabled_only: bool = False) -> list[dict]:
    q = "SELECT * FROM custom_threat_rules WHERE user_id=?"
...
def update_custom_rule(rule_id: int, user_id: int, data: dict) -> dict | None:
    ...
    for k, v in data.items():
        if k in ("name","description","category","title","severity","enabled"):
            fields.append(f"{k}=?"); vals.append(v)
        elif k in ("applies_to","mitigations","tags"):
            fields.append(f"{k}=?"); vals.append(_json.dumps(v))
    if not fields: return get_custom_rule(rule_id)
    fields.append("updated_at=datetime('now')")
    vals += [rule_id, user_id]
    with db_conn(write=True) as c:
        c.execute(f"UPDATE custom_threat_rules SET {', '.join(fields)} WHERE id=? AND user_id=?", vals)
    return get_custom_rule(rule_id)
